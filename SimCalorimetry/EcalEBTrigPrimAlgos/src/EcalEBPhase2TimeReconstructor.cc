...
{
  // test
  //std::cout << "  EcalEBPhase2TimeReconstructor::process(std::vector<int> &addout size  " << addout.size() << std::endl;  
  ...
  for (unsigned int i =0;i<addout.size();i++){
    ...
    if (i == 11) {
      ...
      process();
      //std::cout << "  EcalEBPhase2TimeReconstructor::process(std::vector<int>)    after process() " <<  processedOutput_ << std::endl;
      output[0] = processedOutput_; // >> 6;
      //std::cout << "  EcalEBPhase2TimeReconstructor::process(std::vector<int>)    after setting the output " << output[0] <<  std::endl;
    }
    else if (i == 15){
...
//std::cout << " EcalEBPhase2TimeReconstructor::setParameters time weights after the map  " << params_[0] << " " << params_[1] << " " << params_[2] << " " << params_[3] << " " << params_[4] << " " << params_[5] << " " << params_[6] << " " << params_[7] << " " << params_[8] << " " << params_[9] << " " << params_[10] << " " << params_[11] <<  std::endl;
