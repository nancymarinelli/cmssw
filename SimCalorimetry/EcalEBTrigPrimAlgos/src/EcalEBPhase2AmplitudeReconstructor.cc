...
int EcalEBPhase2AmplitudeReconstructor::setInput(int input) {
  if (input > 0X3FFF) {
    edm::LogError("EcalEBPhase2AmplitudeReconstructor") << "ERROR IN INPUT OF AMPLITUDE FILTER" << std::endl;
    return -1;
  }

  if (inputsAlreadyIn_ < maxSamplesUsed_) {
    if (debug_)
      LogDebug("") << " EcalEBPhase2AmplitudeReconstructor::setInput inputsAlreadyIn_<5 input " << input << std::endl;
    ...
    for (int i = 0; i < (maxSamplesUsed_ - 1); i++) {
      ...
      if (debug_)
        LogDebug("") << " EcalEBPhase2AmplitudeReconstructor::setInput inputsAlreadyIn buffer " << buffer_[i]
                     << std::endl;
    }
...
void EcalEBPhase2AmplitudeReconstructor::process() {
  ...
  for (int i = 0; i < maxSamplesUsed_; i++) {
    ...
    if (debug_)
      LogDebug("") << " AmplitudeFilter buffer " << buffer_[i] << " weight " << weights_[i] << std::endl;
  }
  if (tmpIntOutput < 0)
    tmpIntOutput = 0;
...
void EcalEBPhase2AmplitudeReconstructor::setParameters(uint32_t raw,
  ...
  if (debug_)
    LogDebug("") << " EcalEBPhase2AmplitudeReconstructor::setParameters groupmap size " << groupmap.size()
  ...
  if (it != groupmap.end()) {
    ...
    if (debug_)
      LogDebug("") << " EcalEBPhase2AmplitudeReconstructor::setParameters weights after the map  " << params_[0] << " "
    ...
    if (debug_) {
      for (int i = 0; i < maxSamplesUsed_; ++i) {
        LogDebug("") << " EcalEBPhase2AmplitudeReconstructor::setParameters weights after the cooking " << weights_[i]
                     << std::endl;
      }
      LogDebug("") << std::endl;
    }
