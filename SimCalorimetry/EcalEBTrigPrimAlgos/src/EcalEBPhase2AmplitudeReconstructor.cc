...
{
  // test
  //  std::cout << "  EcalEBPhase2AmplitudeReconstructor::process(std::vector<int> &linout size  " << linout.size() << std::endl;  
...
void EcalEBPhase2AmplitudeReconstructor::process()
{
  ...
  if(tmpIntOutput>0X7FFF)  tmpIntOutput=0X7FFF;
  uint output = tmpIntOutput; // should be 15 bit uint at this point
...
*ecaltpgWeightGroup )
{
  ...
  //std::cout << " EcalEBPhase2AmplitudeReconstructor::setParameters groupmap size " << groupmap.size() << "  channel ID " << raw <<  std::endl;
  ...
  if (it!=groupmap.end()) {
    ...
    //std::cout << " EcalEBPhase2AmplitudeReconstructor::setParameters weights after the map  " << params_[0] << " " << params_[1] << " " << params_[2] << " " << params_[3] << " " << params_[4] << " " << params_[5] << " " << params_[6] << " " << params_[7] << " " << params_[8] << " " << params_[9] << " " << params_[10] << " " << params_[11] <<  std::endl;
