...
void EcalEBPhase2TPParamProducer::analyze(const edm::Event& evt, const edm::EventSetup& evtSetup) {
  ...
  for (unsigned int iGr=0; iGr<nWeightGroups_; iGr++) {
    ampWeights[iGr]=computeWeights(nSamplesToUse_, useBXPlusOne_, phaseShift_,1);
    timeWeights[iGr]=computeWeights(nSamplesToUse_, useBXPlusOne_, phaseShift_,2);
  }
                                                             ...
                                                             int type)  
{
  ...
  double scaleMatrixBy = ((pow(2,15))-1)/(40950+(10*255)); // add a comment here to explain what is this
  ...
  fillFMat(clockSampleSet, useBXPlusOne, sampleSet, sampleDotSet, FMat);
  ...
  for (unsigned int iSample =0;iSample<clockSampleSet.size();iSample++){

    if (type == 1 ) tmpWeightVec.push_back(round(GMat(2,iSample)*multToInt_)); // amp weights
    if (type == 2)  tmpWeightVec.push_back(round(GMat(3,iSample)*multToInt_)); // time weights
  }
...
void EcalEBPhase2TPParamProducer::fillFMat(std::vector<UInt_t> clockSampleSet, bool useThirdPulse, std::vector<float> sampleSet, std::vector<float> sampleDotSet, TMatrix &FMat){
  for (UInt_t i=0;i<clockSampleSet.size();i++){
    Int_t tmpClockToSample = clockSampleSet[i];
...
bool EcalEBPhase2TPParamProducer::computeLinearizerParam(double theta, double gainRatio, double calibCoeff,  int& shift,int& mult){
  ...
  gMatCorr_= pow(2,15)/((4095+255)*10);
  double factor = (1023*(xtal_LSB_*gainRatio*calibCoeff*sin(theta))/(Et_sat_*gMatCorr_))*(1 << shiftTwo_);
